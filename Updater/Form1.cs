...
public partial class Form1 : Form
{
    ...
    private async void Form1_Shown(object sender, EventArgs e)
    {
        ...
        if (await UpdateChecker.CheckUpdaterUpdate(LastVersion))
        {
            await Update(0);
            StatusLabel.Text = "Update updater...";
            await Task.Run(() =>
            {
                UpdateInstaller.InstallUpdater(LastVersion, b =>
                {
                    Update(b);
                });
            });
        ...
        if (!System.IO.File.Exists($"Update.v{LastVersion.Version}{LastVersion.Tag}.7z"))
        {
            StatusLabel.Text = "Download update";
            await UpdateDownloader.DownloadUpdate(LastVersion);
        }
